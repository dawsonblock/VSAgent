...
Tool confirmation policy follows the same rule: the generic renderer never imports `vs/sessions`. Instead it delegates pending-confirmation decisions through `IAgentHostToolConfirmationResolverService`. The default workbench implementation is a no-op. Sessions overrides that service to apply provider-aware policy and approval checks for local and remote agent-host sessions without changing the shared renderer contribution.
