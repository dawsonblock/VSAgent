...
import { IAgentHostToolConfirmationResolverService } from '../../../browser/agentSessions/agentHost/agentHostToolConfirmationResolverService.js';
...
class MockAgentHostService extends mock<IAgentHostService>() {
	...
	private readonly _onDidChangeRootState = new Emitter<IRootState>();
	...
	private _rootStateValue: IRootState | undefined;
	private readonly _rootStateSubscription: IAgentSubscription<IRootState>;

	constructor() {
		super();
		const owner = this;
		this._rootStateSubscription = {
			get value() { return owner._rootStateValue; },
			get verifiedValue() { return owner._rootStateValue; },
			onDidChange: owner._onDidChangeRootState.event,
	...
	override get rootState(): IAgentSubscription<IRootState> {
	...
	setRootAgents(agents: typeof this.agents): void {
		this.agents = agents;
		this._rootStateValue = {
		...
		this._onDidChangeRootState.fire(this._rootStateValue);
	}

	dispose(): void {
		this._onDidAction.dispose();
		this._onDidNotification.dispose();
		this._onDidChangeRootState.dispose();
	}
}
...
function createTestServices(disposables: DisposableStore) {
	...
	const registeredSessionContentProviders = new Map<string, unknown>();
	...
	instantiationService.stub(IChatSessionsService, {
		...
		registerChatSessionContentProvider: (sessionType: string, provider: unknown) => {
			registeredSessionContentProviders.set(sessionType, provider);
			return toDisposable(() => registeredSessionContentProviders.delete(sessionType));
		},
	...
	instantiationService.stub(IAgentHostToolConfirmationResolverService, {
		resolveToolConfirmation: async () => undefined,
	});

	return { instantiationService, agentHostService, chatAgentService, registeredSessionContentProviders };
}
...
suite('AgentHostChatContribution', () => {
	...
	suite('permission requests', () => {
		...
		test('permission_request uses custom confirmation resolution when provided', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionHandler = disposables.add(instantiationService.createInstance(AgentHostSessionHandler, {
				...
				connection: agentHostService,
				connectionAuthority: 'local',
				resolveToolConfirmation: async request => ({
					confirmedReason: { type: ToolConfirmKind.Setting, id: 'chat.tools.autoApprove' },
				}),
			}));

			const { turnPromise, collected, session, turnId, fire } = await startTurn(sessionHandler, agentHostService, disposables);
			fire({ type: 'session/toolCallStart', session, turnId, toolCallId: 'tc-perm-setting', toolName: 'shell', displayName: 'Shell' } as ISessionAction);
			fire({
				type: 'session/toolCallReady', session, turnId, toolCallId: 'tc-perm-setting',
				invocationMessage: 'echo hello', toolInput: 'echo hello',
			} as ISessionAction);
			...
			assert.ok(agentHostService.dispatchedActions.some(a => {
				...
				const action = a.action as IToolCallConfirmedAction;
				return action.toolCallId === 'tc-perm-setting' && action.approved === true && action.confirmed === ToolCallConfirmationReason.Setting;
			}));
		...
		test('permission_request can be denied by custom confirmation resolution', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { instantiationService, agentHostService } = createTestServices(disposables);
			const sessionHandler = disposables.add(instantiationService.createInstance(AgentHostSessionHandler, {
				provider: 'copilot' as const,
				agentId: 'agent-host-copilot',
				...
				resolveToolConfirmation: async request => ({
					...
					reasonMessage: 'Denied by Sessions policy.',
				}),
			}));

			const { turnPromise, collected, session, turnId, fire } = await startTurn(sessionHandler, agentHostService, disposables);
			fire({ type: 'session/toolCallStart', session, turnId, toolCallId: 'tc-perm-policy', toolName: 'write', displayName: 'Write File' } as ISessionAction);
			fire({
				type: 'session/toolCallReady', session, turnId, toolCallId: 'tc-perm-policy',
				invocationMessage: 'Write to /tmp/test.txt',
			} as ISessionAction);
			...
			assert.ok(agentHostService.dispatchedActions.some(a => {
				...
				const action = a.action as IToolCallConfirmedAction;
				return action.toolCallId === 'tc-perm-policy' && action.approved === false && action.reasonMessage === 'Denied by Sessions policy.';
			}));
		...
		test('local contribution routes permission requests through the shared resolver service', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { instantiationService, agentHostService, registeredSessionContentProviders } = createTestServices(disposables);
			let resolverCalls = 0;
			instantiationService.stub(IAgentHostToolConfirmationResolverService, {
				resolveToolConfirmation: async () => {
					resolverCalls++;
					return {
						confirmedReason: { type: ToolConfirmKind.Denied },
						reasonMessage: 'Denied by shared resolver.',
					};
				},
			});
			agentHostService.setRootAgents([{ provider: 'copilot', displayName: 'Agent Host - Copilot', description: 'test', requiresAuth: true }]);

			disposables.add(instantiationService.createInstance(AgentHostContribution));
			await timeout(10);

			const sessionHandler = registeredSessionContentProviders.get('agent-host-copilot') as AgentHostSessionHandler;
			assert.ok(sessionHandler);

			const { turnPromise, collected, session, turnId, fire } = await startTurn(sessionHandler, agentHostService, disposables);
			fire({ type: 'session/toolCallStart', session, turnId, toolCallId: 'tc-local-shared', toolName: 'write', displayName: 'Write File' } as ISessionAction);
			fire({
				type: 'session/toolCallReady', session, turnId, toolCallId: 'tc-local-shared',
				invocationMessage: 'Write to /workspace/file.ts',
			...
			assert.ok(agentHostService.dispatchedActions.some(a => {
				...
				const action = a.action as IToolCallConfirmedAction;
				return action.toolCallId === 'tc-local-shared' && action.approved === false && action.reasonMessage === 'Denied by shared resolver.';
			}));
