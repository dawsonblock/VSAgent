...
import { ChatRequestQueueKind, ConfirmedReason, IChatProgress, IChatService, IChatToolInvocation, ToolConfirmKind } from '../../../common/chatService/chatService.js';
...
export interface IAgentHostSessionHandlerConfig {
	...
	/**
	 * Optional callback that can refine or deny a pending tool confirmation
	 * before the confirmation result is dispatched back to the server.
	 */
	readonly resolveToolConfirmation?: (request: IAgentHostToolConfirmationRequest) => Promise<IAgentHostToolConfirmationResolution | undefined>;
...
export interface IAgentHostToolConfirmationRequest {
	readonly session: URI;
	readonly turnId: string;
	readonly toolCallId: string;
	readonly invocation: IChatToolInvocation;
	readonly confirmedReason: ConfirmedReason;
}

export interface IAgentHostToolConfirmationResolution {
	readonly confirmedReason: ConfirmedReason;
...
export class AgentHostSessionHandler extends Disposable implements IChatSessionContentProvider {
	...
	private _awaitToolConfirmation(
		...
		IChatToolInvocation.awaitConfirmation(invocation, cancellationToken).then(async reason => {
			const resolution = await this._config.resolveToolConfirmation?.({
				session,
				turnId,
				toolCallId,
				invocation,
			...
			const confirmedReason = resolution?.confirmedReason ?? reason;
			const approved = confirmedReason.type !== ToolConfirmKind.Denied && confirmedReason.type !== ToolConfirmKind.Skipped;
			...
			if (approved) {
				this._config.connection.dispatch({
					...
					toolCallId,
					approved: true,
					confirmed: this._toToolCallConfirmationReason(confirmedReason),
				});
			} else {
				this._config.connection.dispatch({
					type: ActionType.SessionToolCallConfirmed,
					...
					toolCallId,
					approved: false,
					reason: confirmedReason.type === ToolConfirmKind.Skipped ? ToolCallCancellationReason.Skipped : ToolCallCancellationReason.Denied,
					...(resolution?.reasonMessage ? { reasonMessage: resolution.reasonMessage } : {}),
				});
			}
		}).catch(err => {
	...
	private _toToolCallConfirmationReason(reason: ConfirmedReason): ToolCallConfirmationReason {
		switch (reason.type) {
