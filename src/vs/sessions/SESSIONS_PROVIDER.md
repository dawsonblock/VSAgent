...
Providers also declare a conservative `capabilities` object. These values are trust inputs for Sessions policy; they do not authorize a provider to bypass policy, approval, or receipt logging.

| Property | Type | Description |
|----------|------|-------------|
| `multipleChatsPerSession` | `boolean` | Whether the provider supports multiple chats inside a single session |
| `hostKind` | `SessionHostKind` | Where the provider primarily hosts execution (`local`, `remote`, `copilot`, `unknown`) |
...
Privileged mutations are expected to submit typed actions through `ISessionActionService`. Provider capabilities are one policy input into that mediated path rather than a direct execution bypass.
...
| `getProviderCapabilities(providerId, sessionId?)` | Resolve the conservative capability set, optionally refined for a concrete session |
| `getProviderMetadata(providerId, sessionId?)` | Resolve descriptive metadata such as remote address or output channel ID |
| `resolveProviderHostKind(providerId, sessionId?)` | Convenience helper for the provider host kind |
