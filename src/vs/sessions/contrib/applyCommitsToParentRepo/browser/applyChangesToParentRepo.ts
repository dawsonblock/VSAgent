...
class ApplyChangesToParentRepoAction extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		try {
			...
			const worktreeBranch = branchResult.kind === SessionActionKind.RunCommand && branchResult.status === SessionActionStatus.Executed && typeof branchResult.value === 'string'
				? branchResult.value
				: undefined;

			if (!worktreeBranch) {
				logService.warn('[ApplyChangesToParentRepo] Could not resolve worktree branch from Sessions action result', branchResult);
			...
			const result = await sessionManagementService.submitAction(activeSession, {
				kind: SessionActionKind.RunCommand,
				requestedBy: SessionActionRequestSource.User,
				command: '_git.mergeBranch',
				args: [repoRoot.fsPath, worktreeBranch],
				cwd: repoRoot,
				launchKind: SessionCommandLaunchKind.Command,
				scope: {
					repositoryPath: repoRoot,
			...
			if (result.status !== SessionActionStatus.Executed) {
				throw new Error(result.summary ?? 'The Sessions action service did not execute the parent-repository merge.');
			}

			const mergeValue = result.kind === SessionActionKind.RunCommand ? result.value : undefined;
			if (!mergeValue) {
				logService.warn('[ApplyChangesToParentRepo] No result from merge command');
			} else {
				notificationService.notify({
					severity: Severity.Info,
					message: typeof mergeValue === 'string' && mergeValue.startsWith('Already up to date')
