...
import { RunCommandActionResult, SessionAction, SessionActionKind, SessionActionRequestSource, SessionActionStatus, SessionCommandLaunchKind } from '../../../../services/actions/common/sessionActionTypes.js';
...
suite('SessionsConfigurationService', () => {
	...
	let submittedActions: { sessionId: string; action: unknown }[];
	...
	setup(() => {
		...
		instantiationService.stub(ISessionsManagementService, new class extends mock<ISessionsManagementService>() {
			...
			override async submitAction(session: ISession, action: SessionAction): Promise<RunCommandActionResult> {
				submittedActions.push({ sessionId: session.sessionId, action });
				return {
					actionId: 'action-1',
	...
	test('runTask submits a mediated run-command action', async () => {
		const session = makeSession({ worktree: worktreeUri, repository: repoUri });

		await service.runTask(makeTask('build', 'npm run build'), session);

		assert.strictEqual(submittedActions.length, 1);
		assert.strictEqual(submittedActions[0].sessionId, session.sessionId);
		assert.deepStrictEqual(submittedActions[0].action, {
			kind: SessionActionKind.RunCommand,
	...
	test('runTask still submits when no cwd is available so policy can deny or receipt it', async () => {
