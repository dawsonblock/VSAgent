...
import { SessionActionKind, SessionActionRequestSource, SessionCommandLaunchKind } from '../../../services/actions/common/sessionActionTypes.js';
...
export interface ISessionsConfigurationService {
	...
	/**
	 * Runs a task through the mediated Sessions action path.
	 */
...
export class SessionsConfigurationService extends Disposable implements ISessionsConfigurationService {
	...
	async runTask(task: ITaskEntry, session: ISession): Promise<void> {
		const repo = this._getSessionRepo(session);
		await this._sessionsManagementService.submitAction(session, {
			kind: SessionActionKind.RunCommand,
			requestedBy: SessionActionRequestSource.User,
			command: this._getTaskCommandPreview(task),
			launchKind: SessionCommandLaunchKind.Task,
			taskLabel: task.label,
			scope: {
				repositoryPath: repo?.uri,
				worktreeRoot: repo?.workingDirectory,
				cwd: repo?.workingDirectory ?? repo?.uri,
			},
		});
	}
	...
	private _getTaskCommandPreview(task: ITaskEntry): string {
