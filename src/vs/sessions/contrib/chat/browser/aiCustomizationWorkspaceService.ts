...
import { SessionActionKind, SessionActionRequestSource, SessionActionStatus, SessionCommandLaunchKind } from '../../../services/actions/common/sessionActionTypes.js';
...
export class SessionsAICustomizationWorkspaceService implements IAICustomizationWorkspaceService {
	...
	private async commitFileToRepos(fileUri: URI, repositoryUri: URI, worktreeUri: URI | undefined): Promise<void> {
		...
		try {
			if (repoFileUri.toString() !== fileUri.toString()) {
				await this.copyFileIntoTarget(session, fileUri, repoFileUri, repositoryUri, worktreeUri);
			}
			await this.runSessionsCommand(session,
				'github.copilot.cli.sessions.commitToRepository',
				[{ repositoryUri, fileUri: repoFileUri }],
				{
					repositoryPath: repositoryUri,
					worktreeRoot: worktreeUri,
					files: [repoFileUri],
					cwd: repositoryUri,
				}
			);
		...
		if (worktreeUri) {
			...
			try {
				if (worktreeFileUri.toString() !== fileUri.toString()) {
					await this.copyFileIntoTarget(session, fileUri, worktreeFileUri, repositoryUri, worktreeUri);
				}
				await this.runSessionsCommand(session,
	...
	private async commitDeletionToRepos(fileUri: URI, repositoryUri: URI, worktreeUri: URI | undefined): Promise<void> {
		...
		try {
			if (await this.fileService.exists(repoFileUri)) {
				await this.deleteFileFromTarget(session, repoFileUri, repositoryUri, worktreeUri);
			}
			await this.runSessionsCommand(session,
				'github.copilot.cli.sessions.commitToRepository',
				[{ repositoryUri, fileUri: repoFileUri }],
	...
	private async copyFileIntoTarget(session: ISession, source: URI, target: URI, repositoryUri: URI, worktreeUri: URI | undefined): Promise<void> {
		const content = await this.fileService.readFile(source);
		const result = await this.sessionsService.submitAction(session, {
			kind: SessionActionKind.WritePatch,
			requestedBy: SessionActionRequestSource.User,
			patch: '',
			files: [target],
			operations: [{ resource: target, contents: content.value.toString() }],
			scope: {
				repositoryPath: repositoryUri,
				worktreeRoot: worktreeUri,
				files: [target],
		...
		if (result.status !== SessionActionStatus.Executed) {
			throw new Error(result.summary ?? `Could not replicate customization file into '${target.toString()}'.`);
		}
	}

	private async deleteFileFromTarget(session: ISession, target: URI, repositoryUri: URI, worktreeUri: URI | undefined): Promise<void> {
		const result = await this.sessionsService.submitAction(session, {
			kind: SessionActionKind.WritePatch,
			requestedBy: SessionActionRequestSource.User,
			patch: '',
			files: [target],
			operations: [{ resource: target, delete: true, useTrash: true }],
			scope: {
				repositoryPath: repositoryUri,
				worktreeRoot: worktreeUri,
				files: [target],
		...
		if (result.status !== SessionActionStatus.Executed) {
			throw new Error(result.summary ?? `Could not delete customization file '${target.toString()}'.`);
		}
	}

	private async runSessionsCommand(session: ISession, command: string, args: readonly unknown[], scope: {
		repositoryPath: URI;
		worktreeRoot?: URI;
		files: readonly URI[];
		cwd: URI;
		...
		if (result.status !== SessionActionStatus.Executed) {
			throw new Error(result.summary ?? `Could not execute Sessions command '${command}'.`);
		}
	}
