...
Authority note: customizations, prompts, and hidden mode instructions are advisory inputs only. They can influence planning and UI affordances, but they must not widen write, command, git, or worktree authority. Privileged execution is mediated separately by the Sessions action service, and migration of older customization-side mutation paths happens incrementally.
